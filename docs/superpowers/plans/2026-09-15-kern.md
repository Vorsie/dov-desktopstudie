...
CATALOGUE: List[MapEntry] = [
    ...
    MapEntry("dhmv_dtm", "ligging", "Digitaal Hoogtemodel Vlaanderen II - DTM 1 m",
             "https://geo.api.vlaanderen.be/DHMV/wms", "DHMVII_DTM_1m", "Digitaal Vlaanderen - DHMV II", opacity=0.6, legend=True),
    # --- historische kaarten ---
    ...
    _hist("vandermaelen", "Topografische kaart Vandermaelen (1846-1854)", "https://geo.api.vlaanderen.be/HISTCART/wms", "vandermaelen"),
...
def _map_pages(chapter: str, **kw) -> List[Page]:
    return [MapPage(e.id, e.title, legend=e.legend, note=e.note, **kw) for e in catalogue.entries(chapter)]
...
def build_report(result: StudyResult, meta: ReportMeta) -> Report:
    ...
    for model, borehole in result.virtual_boreholes.items():
        ...
        vb.pages.append(TablePage(f"Virtuele boring op het zwaartepunt - {MODEL_TITLES.get(model, model)}",
    ...
    inv.pages.append(MapPage("grb", "Overzicht beschikbaar grondonderzoek", extent_factor=1.0, show_investigations=True, show_section_line=True))
        ...
        for entry in wanted:
            ...
            def fetch(e=entry) -> None:
                if e.fact_mode == "wfs":
                    ...
                    rows = []
                    seen = set()
                    for x, y in [self.zone.centroid] + list(self.zone.ring[:8]):
