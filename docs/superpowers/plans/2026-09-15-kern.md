...
> **Status 2026-09-15: uitgevoerd** (taken 0–16 plus 11b en 14b, gereviewd per bundel; 202+ tests, CI groen op Python 3.9 en 3.12). De secties *Afwijkingen bij uitvoering* onder de taken beschrijven waar de code van de oorspronkelijke tekst afwijkt.
...
- [x] **Step 1: Schrijf het opnamescript**
...
- [x] **Step 3: Commit (script apart, fixtures samen als één logische eenheid)**
...
- [x] **Step 1: Schrijf de importbewakingstest**
...
- [x] **Step 2: Run, verwacht FAIL (module ontbreekt)**
...
- [x] **Step 3: Maak de pakketbestanden**
...
- [x] **Step 5: Implementeer logging_util**
...
- [x] **Step 6: Schrijf conftest met FixtureClient**
...
- [x] **Step 7: Run alle tests, verwacht PASS**
...
- [x] **Step 2: Run, verwacht FAIL** — `pytest tests/core/test_geometry.py -v` → `ImportError`.

- [x] **Step 3: Implementeer geometry.py**
...
- [x] **Step 4: Run, verwacht PASS** — `pytest tests/core/test_geometry.py -v` → 9 passed.
...
- [x] **Step 2: Run, verwacht FAIL** — `ImportError`.

- [x] **Step 3: Implementeer model.py**
...
- [x] **Step 4: Run, verwacht PASS** — 3 passed.
...
- [x] **Step 3: Implementeer catalogue.py**
...
- [x] **Step 4: Run, verwacht PASS** — 5 passed.
...
- [x] **Step 4: Run, verwacht PASS** — 4 passed.
...
- [x] **Step 3: Implementeer geocoder.py**
...
- [x] **Step 4: Run, verwacht PASS** — 2 passed, 1 deselected (live).
...
- [x] **Step 3: Implementeer dov_wfs.py**
...
- [x] **Step 4: Run, verwacht PASS** — 4 passed, 1 deselected.

- [x] **Step 5: Live-check** — `pytest tests/core/test_dov_wfs.py -m live -v` → 1 passed.
...
- [x] **Step 3: Implementeer dov_xml.py**
...
- [x] **Step 4: Run, verwacht PASS** — 6 passed. Als `test_coded_lithology_layers_keep_raw_codes` faalt op het aantal (54): tel `<laag>` in de fixture (`grep -c "<laag>"`) en pas de verwachting aan het echte bestand aan — de fixture is de waarheid.
...
- [x] **Step 3: Implementeer virtuele_boring.py**
...
- [x] **Step 4: Run, verwacht PASS** — 3 passed, 1 deselected.
...
- [x] **Step 3: Implementeer wms_gfi.py**
...
- [x] **Step 4: Run, verwacht PASS** — 2 passed.
...
- [x] **Step 3: Implementeer section.py**
...
- [x] **Step 1: Schrijf de falende tests (in de woorden van de regels)**
...
- [x] **Step 3: Implementeer checks.py**
...
- [x] **Step 4: Run, verwacht PASS** — 8 passed.
...
- [x] **Step 3: Implementeer common.py**
...
- [x] **Step 4: Implementeer cpt_figure.py**
...
- [x] **Step 5: Implementeer borehole_column.py**
...
- [x] **Step 6: Implementeer vb_column.py**
...
- [x] **Step 7: Implementeer section_figure.py**
...
- [x] **Step 8: Run, verwacht PASS** — 3 passed. **Bekijk daarna de PNG's** in de pytest-tmp-map (`tmp_path` staat in de testoutput met `-s`, of schrijf ze tijdelijk naar `uitvoer/`): qc-as bovenaan, diepte omlaag, kolommen leesbaar, doorsnede met maaiveldlijn en gearceerde zone.
...
**Afwijkingen bij uitvoering (2026-09-15, na review):** `None`-maaiveld → `nan` vóór het plotten; titel meldt mislukte doorsnedepunten; `new_figure`/`new_figure_grid` in `figures/common.py` maken figuren zonder pyplot; CPT-titel toont het maaiveld met twee decimalen; kolomtitel valt terug op `Virtuele boring <model>`.
...
- [x] **Step 3: Implementeer report_content.py**
...
**Afwijkingen bij uitvoering (2026-09-15, na live-controle):** het profiel heeft wél een exact datum: `minValue` + som van de diktes per kolom = maaiveld (op de centimeter gelijk aan de doorprik; `profile_surface_at`), anker-interpolatie is alleen terugval; onbekende laagcodes worden op numeriek suffix ingeschoven (niet achteraan); constante heet `VB_PROFILE_URL`; `parse_profile(..., resolution_m)`; `geometry.interpolate`/`geometry.chainages` gedeeld door section en figuur; `draw_depth_column` geeft `(drawn_depth, skipped)` terug (weggelaten labels worden op de figuur vermeld); `label_chars` 48; stapelvolgorde komt van het eerste anker (numeriek inschuiven maakt het dichtstbijzijnde anker overbodig); `INV` telt mee in het datum (bodemopvulling bij `knownLowBoundary`), `maxValue` is de zelfcontrole; kolommen buiten het model worden overgeslagen en gelogd; zone-markering als band boven het maaiveld + gestippelde grenslijnen (geen verkleuring van de geologie).
...
- [x] Catalogus (hoofdstuk `geologie`): `grondverschuiving_gevoeligheid` (WMS `grondverschuivingen:grndversch_gevoeligh`, WFS idem, velden `gevoelighd`, `klasse`, labels, scale 25000), `grondverschuiving_gekarteerd` (WMS/WFS `grondverschuivingen:grndversch_gekarteerd`, velden `type`, `naam`, `gemeente`, `helling`, `rapport`, scale 10000), `pfas_no_regret` (WMS `pfas:no_regret_zones`, WFS `pfas:no_regret_huidig`, velden `pfasdossiernr`, `gemeente`, `straat`, `nrm_status_zone`, `zone_geldig_vanaf`, `no_regret_maatregelen`, scale 10000); hoofdstuk `historisch`: `bommenkaart` (enabled=False, note: "Bommenkaart.be (Bom-Be BV) is geen open data en heeft geen WMS/WFS; raadpleeg de kaart manueel en vermeld het CTE-risico (WOI/WOII) in de studie").
- [x] Fixtures: `wfs_grndversch_gevoeligh_intersects.json`, `wfs_grndversch_gekarteerd_intersects.json`, `wfs_pfas_no_regret_intersects.json` (met `propertyName`, zones hierboven); `test_fact_fields_exist_in_recorded_fixtures` uitgebreid.
- [x] Signaleringen: `grondverschuiving_gevoelig` (klasse ≥ 2 of `gevoelighd` bevat "matig"/"hoog"), `grondverschuiving_gekarteerd` (≥ 1 rij), `pfas_no_regret` (≥ 1 rij; feit noemt dossiernummer(s) en status; advies: no-regret-maatregelen en OVAM-richtlijnen voor grondverzet raadplegen). Tests in de woorden van de regel + negatieve gevallen.
- [x] Rapport: hoofdstuk 2 krijgt naast de NGI-tekst ook de bommenkaart-tekst (via het uitgeschakelde slot, geen extra code); de manuele-controletekst vermeldt CTE-risico.

**Afwijkingen bij uitvoering (2026-09-15, na live-controle):** `pfas:no_regret_zones` is een WMS-*stijl*, geen laag → WMS én WFS gebruiken `pfas:no_regret_huidig`; `gevoelighd`-waarden zijn "lage/matige/hoge gevoeligheid" → `LANDSLIDE_WORD_CLASSES` (laag=1, matig=2, hoog/hoge=3); gevoeligheids-fixture (klasse 1) dient als negatief geval; PFAS-rijen zonder dossiernummer worden "zonder dossiernummer" gemeld.

### Task 15: Orchestrator `study.py`

**Afwijkingen bij uitvoering (2026-09-15, na review):** `_load_each` (per-item isolatie in
threads); `StudyCancelled`/`should_cancel`; `_gfi_points` (gelijkmatig over de ring);
`_note_municipality` (dichtstbijzijnde feature); `_truncation_signals` + `_section_signals`;
signaleringen na de figuren; bewaakte JSON-schrijfstap; `guarded` met korte foutboodschap; lege
virtuele boringen/profiel/doorsnede als niet-beschikbaar.
...
- [x] **Step 3: Implementeer study.py**
...
- [x] **Step 4: Run, verwacht PASS** — 2 passed. Draai daarna de hele suite: `pytest tests/core -v` → alles groen; `ruff check .` → geen meldingen (los import-volgorde op met `ruff check --fix`).
...
**Afwijkingen bij uitvoering (2026-09-15, na review):** `run_core.py` met `--straal`/`--cache`,
exitcodes 2 (adres) en 3 (bron mislukt), `log.child("study")`; CI-matrix 3.9/3.12 met pip-cache;
`ruff==0.16.*`.
...
- [x] **Step 2: Draai live voor Gent en voor een landelijke locatie**
...
- [x] **Step 3: Schrijf de CI-workflow**
...
- [x] **Step 4: Werk CHANGELOG en CLAUDE.md bij**
