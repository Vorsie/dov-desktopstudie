...
| `desktopstudie/core/services/dov_wfs.py` | `DovWfs`: DescribeFeatureType, GetFeature met paging, DWITHIN/INTERSECTS |
...
def parse_groundwater_levels(xml_bytes: bytes) -> List[GwLevel]:
    root = ET.fromstring(xml_bytes)
    levels: List[GwLevel] = []
    for pm in root.iter("{*}peilmeting"):
...
def check_anthropogenic(result: StudyResult) -> List[Signalering]:
    ...
    for layer in vb.layers:
        if layer.name.lower().startswith("antropogeen"):
...
def check_shallow_tertiary(result: StudyResult) -> List[Signalering]:
    ...
    if vb is None or vb.surface_mtaw is None:
        return []
    quaternary = [layer for layer in vb.layers if layer.name.lower().startswith("quartair")]
    if not quaternary:
        return []
...
from .model import Borehole, Cpt, GwFilter, MapFact, Provenance, StudyResult, StudyZone
...
class _Runner:
    ...
    def run(self) -> StudyResult:
        ...
        self.result.signaleringen = checks.run_all(self.result)
