...
def plot_section(section: Section, path: Path, max_depth_m: float = 60.0) -> Path:
    ...
    for p in section.projected:
        z = p.z_mtaw if p.z_mtaw is not None else (max(s for s in surfaces if s is not None) if any(surfaces) else 0.0)
...
def test_run_produces_result_json_and_figures(gent_ring, tmp_path):
    ...
    assert set(result.virtual_boreholes) == {"g3dv3_L", "g3dv3_P", "hcovv2_S"}
...
class Settings:
    ...
    models_centroid: Tuple[str, ...] = ("g3dv3_L", "g3dv3_P", "hcovv2_S")
...
class _Runner:
    ...
    def section(self) -> None:
        ...
        self.result.section = build_section(self.client, self.zone, self.result.cpts, self.result.boreholes, self.result.gw_filters,
                                            self.s.n_section_points, self.s.corridor_m, self.s.model_section)
