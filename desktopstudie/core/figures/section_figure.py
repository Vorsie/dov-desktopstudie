...
missing, those anchors themselves as wide columns. Surface line through the column tops, zone
extent shaded, projected investigations as vertical markers."""
...
HEADROOM_M = 5.0  # air above the highest surface, so the labels of projected tests have room
...
def _draw_surface_line(ax, xs: Sequence[float], surfaces: Sequence[Optional[float]], half_width: float):
    """The surface through the column tops, extended flat to the outer edge of the first and last
    column so it spans the full drawn width rather than stopping at the outermost centre."""
    plot_surfaces = [s if s is not None else float("nan") for s in surfaces]
    if xs:
        line_xs = [xs[0] - half_width] + list(xs) + [xs[-1] + half_width]
        line_ys = [plot_surfaces[0]] + plot_surfaces + [plot_surfaces[-1]]
    else:
        line_xs, line_ys = [], []
    handle, = ax.plot(line_xs, line_ys, color="black", lw=1.2, label=SURFACE_LABEL)
    return handle
...
def _draw_anchors(ax, section: Section, xs: Sequence[float], surfaces: Sequence[Optional[float]],
    ...
    for x, vb in zip(_anchor_chainages(section), section.boreholes):
        ...
        drawn, = ax.plot([x, x], [top, top - max_depth_m], color="#333333", lw=0.7, ls="--",
                         label=ANCHOR_LABEL)
        handle = handle or drawn
    return handle


def _draw_legend(ax, surface_line, zone, anchor_line, seen: Dict[str, str]) -> None:
    handles = [surface_line, zone]
...
def _build_section_figure(section: Section, max_depth_m: float = 60.0):
    ...
    if has_profile:
        ...
        half_width = profile.resolution_m / 2.0
        # no visible outline: at profile resolution the columns have to read as continuous bands
        seen = _draw_columns(ax, xs, surfaces, [column.layers for column in profile.columns],
                             half_width, max_depth_m, lw=0.0)
    else:
        xs = _anchor_chainages(section)
        surfaces = [vb.surface_mtaw for vb in section.boreholes]
        half_width = _column_half_width(xs)
        seen = _draw_columns(ax, xs, surfaces, [vb.layers for vb in section.boreholes], half_width,
                             max_depth_m, lw=0.3)
    surface_line = _draw_surface_line(ax, xs, surfaces, half_width)
    zone = ax.axvspan(section.zone_from_m, section.zone_to_m, color="red", alpha=0.08, label=ZONE_LABEL)
    ...
    _draw_legend(ax, surface_line, zone, anchor_line, seen)
