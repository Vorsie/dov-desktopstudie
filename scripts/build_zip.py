...
`__pycache__` and compiled files. The tests live outside the package, so a `tests` folder is
skipped only as a safeguard. The version is read from metadata.txt, the one place it is declared.
...
def main(argv=None) -> int:
    ...
    print(f"{target} ({len(members())} bestanden)")
    return 0
